...
- v1 Evidenceの69 Scenario Row対応をbounded補助Evidenceとして維持し、公開済み29 Behavior × 10 Scenarioの290専用Proof Fileへ分離した。Security trancheの4 rowはPostgreSQL/psql 18.6、全Variant、retry 0、専用Oracle、Source/Harness digest、SQL/plan/WAL/log/metricを同一実行へ固定した。strict Closureは4、Gapは286、Authority atomic bindingは0である
...
- 統合Reference Systemは固定PostgreSQL/psql 18.6上で10 Scenarioを10/10実行し、SQL、Plan、WAL、Server log、Metricを保存する。統合成功は290個のBehavior固有Proofへ流用しない。Scenario Closure PlanはPostgreSQL固有分母から残り286 rowをrisk順79 tranche（各最大4 row）へ分割した。Backup/PITR/Replication/Upgrade統合、複数方式Comparison、Runbook演習、現行Definitive Certificateは未実装
...
- Core非後退Gate: baseline 239 item / current 1,250 item / approved Replacement 3件 / pass。Core v1.3 capture契約は同一公開commitを再捕捉し、旧全項目一致に加えて公開時点Scenario Closure Plan / Evidence Durability 0件を確認した
