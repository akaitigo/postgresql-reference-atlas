...
- FE copyright-safe locator設計を適用し、第三者本文を保存せず11,340候補のURL・metadata・document/context digest・byte offsetを固定。Core広域Source rootのfetch failed 10 / locator deferred 10、公式Docs URL再取得deferred 183、Human review 0、Authority本文全体exhaustive falseを明記
...
各v1 EvidenceはSource Lock、Environment Manifest、Harness Manifest、Artifact DigestとSizeを維持する。`make authority-locator-verify`はLocator artifactへの第三者本文field混入を拒否し、stale/deferred/Human review集計と未Closure境界を検査する。`make definitive-audit`は分類完全性とGap Ledgerを検査し、`make definitive-gate`は全Proofが閉じるまで失敗する。
