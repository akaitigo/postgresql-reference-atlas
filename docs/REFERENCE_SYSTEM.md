...
各rowはserver、client、version、runtime identityと、SQL、plan、WAL、log、metric Artifactを独立に評価する。専用Evidenceから確認できる項目だけを`observed`とし、それ以外は理由付き`gap`とする。統合Reference Systemのidentity、log、plan、WAL、metricはCross-behavior参照として全rowへ接続するが、Behavior固有Proofへ昇格しない。現行reportはSecurity 4 rowを専用PostgreSQL/psql 18.6で初回実行し、残り286 rowをgapとして保持する。
