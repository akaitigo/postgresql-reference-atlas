...
各rowはserver、client、version、runtime identityと、SQL、plan、WAL、log、metric Artifactを独立に評価する。専用Evidenceから確認できる項目だけを`observed`とし、それ以外は理由付き`gap`とする。統合Reference Systemのidentity、log、plan、WAL、metricはCross-behavior参照として全rowへ接続するが、Behavior固有Proofへ昇格しない。現行reportはSecurity 12 rowを固定PostgreSQL/psql runtimeで初回実行し、残り278 rowをgapとして保持する。version compatibilityは14.24〜18.6、pg_upgradeとlogical upgradeは17.11→18.6、backup/restore、SIGKILL recovery、logical replicationは18.6を専用runtimeで検証する。
