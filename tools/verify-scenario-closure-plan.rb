...
abort "Next risk tranche" unless actual.dig("next_tranche", "id") == "security-001" && actual.dig("next_tranche", "row_ids") == SecurityScenarioTranche.expected_row_ids
...
SecurityScenarioTranche.verify_next_runtime_tranche!(actual)
...
puts "Verified PostgreSQL Scenario Closure Plan: remaining=#{summary.fetch('remaining_rows')} completed=#{summary.fetch('completed_dedicated_rows')} tranches=#{summary.fetch('planned_tranches')} maximum_rows=4 next=security-001 next_runtime=refusal-001"
