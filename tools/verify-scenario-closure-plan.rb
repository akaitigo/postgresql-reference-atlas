...
abort "PostgreSQL Scenario denominator drift" unless summary.fetch("remaining_rows") == 286 && summary.fetch("by_scenario").fetch("security") == 25 && summary.fetch("by_scenario").reject { |scenario, _| scenario == "security" }.values == Array.new(9, 29)
abort "PostgreSQL completed suite overclaim" unless summary.fetch("completed_dedicated_rows") == 4 && actual.fetch("completed_rows").length == 4 && actual.fetch("completed_rows").all? { |row| row.fetch("scenario") == "security" && row.fetch("all_first_attempt_pass") && row.fetch("all_trace_streams") && row.fetch("variant_ids") == ["postgresql-verification-matrix-v2"] }
abort "Tranche contract" unless summary.fetch("planned_tranches") == 79 && actual.fetch("tranches").all? { |tranche| tranche.fetch("pattern_rows").between?(1, 4) && tranche.fetch("variant_runs") >= tranche.fetch("pattern_rows") }
