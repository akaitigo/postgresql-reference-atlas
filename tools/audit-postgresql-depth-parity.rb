...
errors << "PostgreSQL strict Scenario closure" unless scenario_summary.fetch("pattern_specific_rows") == 8 && scenario_summary.fetch("pattern_specific_runtime_rows") == 8 && scenario_summary.fetch("pattern_specific_gaps") == 282
...
errors << "PostgreSQL Scenario Closure Plan denominator" unless closure_plan_summary.fetch("remaining_rows") == 282 && closure_plan_summary.fetch("completed_dedicated_rows") == 8 && closure_plan_summary.fetch("planned_tranches") == 78
...
errors << "Atomic Evidence runtime report" unless scenario_runtime_report.fetch("status") == "passed" && scenario_runtime_report.fetch("retention_contract") == {"publish_on"=>"full-run-passed", "failed_run"=>"retain-prior-success", "swap"=>"staged-directory-rename-with-rollback"} && scenario_runtime_report.fetch("tests").length == 8
