...
batches.each do |batch|
  File.write(File.join(directory, "#{batch.fetch('batch_id')}.json"), AuthorityReviewQueue.canonical_json(batch) + "\n")
end
FileUtils.mkdir_p(File.dirname(File.join(root, AuthorityReviewQueue::LEDGER_PATH)))
File.write(File.join(root, AuthorityReviewQueue::LEDGER_PATH), AuthorityReviewQueue.canonical_json(ledger) + "\n")
File.write(File.join(root, AuthorityReviewQueue::INDEX_PATH), AuthorityReviewQueue.canonical_json(index) + "\n")
